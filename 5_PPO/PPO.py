    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        if i_episode % print_interval == 0:
            ...
            torch.save(
                ...
                f"checkpoints/ppo_policy_model_{i_episode}.pt",
            )
            torch.save(
                agent.value_network.state_dict(),
                f"checkpoints/ppo_value_model_{i_episode}.pt",
            )
...
def test():
    ...
    agent.policy_network.load_state_dict(
        torch.load(f"checkpoints/ppo_policy_model_{last_episode}.pt")
    )
    agent.value_network.load_state_dict(
        torch.load(f"checkpoints/ppo_value_model_{last_episode}.pt")
    )
